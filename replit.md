...
## Overview
AI-powered credit report analysis and dispute workflow platform. Users can upload credit reports for automated analysis, or manually enter negative accounts and walk through a guided dispute workflow with AI-powered violation detection and letter generation.
...
5. **Dispute Letter Generation**: AI-generated letters (initial dispute, validation request, follow-up, intent to sue)
6. **Workflow Tracking**: 4-step workflow (Start → Add Accounts → Classify → Next Steps)
7. **Profile Clarity View**: Summary dashboard of all accounts, violations, and letters
...
- `server/ai-services.ts` - Account-level AI services (violation detection + letter generation)
- `server/routes.ts` - API endpoints (reports, scans, accounts, violations, letters)
...
- `GET /api/scans/:id` - Get scan with accounts, violations, letters
...
- `POST /api/accounts/:id/generate-letter` - AI letter generation

### Letters
- `PATCH /api/letters/:id` - Update letter content/status
...
- `letters` - AI-generated dispute letters
...
- Workflow steps: pending → classified → scanned → letter_generated → letter_sent → follow_up
