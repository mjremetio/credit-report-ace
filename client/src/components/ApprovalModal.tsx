    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-2xl max-w-md w-full mx-4 shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="bg-background border border-border rounded-lg p-4 space-y-2">
            ...
            <div className="flex justify-between text-xs font-mono">
              <span className="text-green-400">{summary.confirmed} violations confirmed</span>
            </div>
            <div className="flex justify-between text-xs font-mono">
              <span className="text-yellow-400">{summary.modified} violation{summary.modified !== 1 ? "s" : ""} modified</span>
            </div>
            <div className="flex justify-between text-xs font-mono">
              <span className="text-red-400">{summary.rejected} violation{summary.rejected !== 1 ? "s" : ""} rejected (excluded from export)</span>
            </div>
            <div className="border-t border-border/50 pt-2 mt-2 flex justify-between text-xs font-mono">
              <span className="text-white font-medium">Total in final report: {totalInReport} violations</span>
            </div>
          </div>
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-border">
          <button
            onClick={onClose}
            className="flex-1 px-4 py-3 bg-secondary border border-border text-muted-foreground rounded-lg hover:text-white transition-colors font-mono text-sm"
          >
            Cancel
          </button>
          <button
            ...
            className="flex-1 px-4 py-3 bg-primary text-black font-medium rounded-lg hover:bg-primary/90 transition-colors disabled:opacity-50 inline-flex items-center justify-center gap-2 font-mono text-sm"
          >
